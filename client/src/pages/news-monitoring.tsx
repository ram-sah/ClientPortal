...
export default function NewsMonitoring() {
  ...
  const {
    ...
    queryKey: selectedBrandId 
      ? ["/api/news-monitoring/airtable", "brandId", selectedBrandId]
      : ["/api/news-monitoring/airtable"],
    queryFn: async ({ queryKey }) => {
      // Extract brand filtering from queryKey
      const hasFilter = queryKey.length > 1 && queryKey[1] === "brandId";
      const brandId = hasFilter ? queryKey[2] as string : null;
      
      // Construct URL with query parameters
      const baseUrl = "/api/news-monitoring/airtable";
      const url = brandId ? `${baseUrl}?brandId=${encodeURIComponent(brandId)}` : baseUrl;
      
      // Use the same auth approach as the default queryFn
      ...
      if (!res.ok) {
        throw new Error(`Failed to fetch news monitoring data: ${res.status} ${res.statusText}`);
      }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {showSuccess && (
              <div
                className="flex items-center gap-2 text-green-600 animate-fade-in"
        ...
        <div className="bg-white rounded-lg border p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900" data-testid="brand-filter-title">
              Filter by Brand
            </h3>
            <Badge variant="secondary" data-testid="active-records-count">
              {newsMonitoringData.length} records
            </Badge>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
              onClick={() => setSelectedBrandId("")}
              variant={selectedBrandId === "" ? "default" : "outline"}
              size="sm"
              className="flex items-center gap-2"
            ...
            {isLoadingBrands ? (
              ...
              brandsData.map((brand) => (
                <Button
                  ...
                  size="sm"
                  className="flex items-center gap-2"
                  data-testid={`brand-filter-${brand.name.toLowerCase().replace(/\s+/g, '-')}`}
                >
                  {brand.name}
                </Button>
              ))
        ...
        {isRefreshing && (
          <div
            className="bg-white rounded-lg border p-4 animate-fade-in"
                ...
                >
                  <CardHeader>
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <Badge
                            ...
                            data-testid={`news-content-type-${index}`}
                          >
                            {newsItem.contentType}
                          </Badge>
                          <Badge
                            className="bg-indigo-100 text-indigo-800 hover:bg-indigo-100"
                            data-testid={`news-trend-tag-${index}`}
                          >
                          ...
                          {newsItem.publicationDate && (
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                ...
                                data-testid={`news-publication-date-${index}`}
                              >
                                Publication Date:{" "}
                                {new Date(
                                  newsItem.publicationDate,
                      ...
                      <div
                        className={`text-center p-4 rounded-lg ${getScoreBgColor(newsItem.totalScore)}`}
                      >
                        <div
                          className={`text-3xl font-bold ${getScoreColor(newsItem.totalScore)}`}
                          data-testid={`news-total-score-${index}`}
                        >
                          {newsItem.totalScore}%
                        </div>
                  ...
                  <CardContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                      <div className="text-center p-4 bg-green-50 rounded-lg transition-all duration-300 hover:shadow-md">
                        ...
                        <div
                          ...
                          data-testid={`sentiment-score-${index}`}
                        >
                      ...
                      <div className="text-center p-4 bg-blue-50 rounded-lg transition-all duration-300 hover:shadow-md">
                        ...
                        <div
                          ...
                          data-testid={`relevance-score-${index}`}
                        >
                      ...
                      <div className="text-center p-4 bg-purple-50 rounded-lg transition-all duration-300 hover:shadow-md">
                        ...
                        <div
                          ...
                          data-testid={`authority-score-${index}`}
                        >
                        ...
                        <Progress
                          value={newsItem.sourceAuthorityScore}
                          className="h-2 mt-2 transition-all duration-500"
                        />
                      </div>
                      ...
                      <div className="text-center p-4 bg-orange-50 rounded-lg transition-all duration-300 hover:shadow-md">
                        ...
                        <div
                          ...
                          data-testid={`engagement-score-${index}`}
                        >
                        ...
                        <Progress
                          value={newsItem.engagementScore}
                          className="h-2 mt-2 transition-all duration-500"
                        />
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      <p
                        ...
                        data-testid={`recommended-actions-${index}`}
                      >
          ...
          <Card className="border-border/60">
            <CardContent className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">
                We couldn't find any news monitoring data. This could be
                because:
              </p>
